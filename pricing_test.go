...
func TestGetPricingKnownModel(t *testing.T) {
	p := getPricing("claude-sonnet-4-6")
...
func TestGetPricingPrefixMatch(t *testing.T) {
	// versioned model ID that isn't an exact key should still match
	p := getPricing("claude-opus-4-7-20260101")
	if p.Input != 5.0 {
		t.Errorf("prefix match failed, got input price %f want 5.0", p.Input)
	}
}

func TestGetPricingUnknownFallsBackToSonnet(t *testing.T) {
	p := getPricing("claude-unknown-model-99")
	if p.Input != 3.0 {
